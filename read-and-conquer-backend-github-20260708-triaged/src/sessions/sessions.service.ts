...
export class SessionsService {
  ...
  async start(userId: number, dto: StartSessionDto) {
    ...
    this.ensureGpsAccuracy(dto.accuracyMeters);
  ...
  async submit(userId: number, sessionId: number, dto: SubmitSessionDto) {
    ...
    if (session.status !== ReadingSessionStatus.IN_PROGRESS) {
      throw new BusinessException(
        BusinessCode.SESSION_NOT_ACTIVE,
        'Only in-progress sessions can be submitted',
      );
    }
    ...
    if (session.book.totalPages && dto.endPage > session.book.totalPages) {
      throw new BusinessException(
        BusinessCode.INVALID_PAGE_RANGE,
        'End page cannot exceed total pages',
      );
    }
    const now = new Date();
    ...
    if (durationMinutes < MIN_READING_MINUTES) {
      throw new BusinessException(
        ...
        '최소 20분 이상 독서해야 인증 정보를 제출할 수 있습니다.',
      );
    }
    if (!this.validatePageProgress(dto.startPage, dto.endPage, durationMinutes)) {
      throw new BusinessException(
        BusinessCode.INVALID_PAGE_RANGE,
        'Reading pages are too high for the elapsed time',
      );
    }
    ...
    return {
      sessionId,
      status: ReadingSessionStatus.SUBMITTED,
      readPages: dto.endPage - dto.startPage + 1,
  ...
  async cancel(userId: number, sessionId: number) {
    ...
    const canceled = await this.prisma.readingSession.update({
      where: { sessionId },
      data: {
        endTime: new Date(),
        status: ReadingSessionStatus.CANCELED,
        failReason: '사용자가 세션을 취소했습니다.',
      },
    });
  ...
  async completeBySessionId(
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        const finalizedStatuses: ReadingSessionStatus[] = [
          ReadingSessionStatus.VERIFICATION_PASSED,
          ReadingSessionStatus.VERIFICATION_FAILED,
          ReadingSessionStatus.CANCELED,
        ];
        if (finalizedStatuses.includes(session.status)) {
          throw new BusinessException(
            BusinessCode.SESSION_NOT_ACTIVE,
        ...
        const ai = coverImageUrl && reviewText
          ...
          : session.aiVerification
            ...
            : {
                ...
                llmConfidence: 0,
                isPassed: false,
                failReason: 'AI verification has not been submitted',
              };
  ...
  private ensureGpsAccuracy(accuracyMeters?: number) {
    if (
      accuracyMeters !== undefined &&
      accuracyMeters > MAX_GPS_ACCURACY_METERS
    ) {
      throw new BusinessException(
        BusinessCode.GPS_ACCURACY_TOO_LOW,
        'GPS 정확도가 낮습니다. 위치를 다시 측정해 주세요.',
      );
    }
  }
