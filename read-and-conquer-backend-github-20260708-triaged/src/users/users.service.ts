...
export class UsersService {
  ...
  async checkNickname(nickname: string) {
    ...
    this.validateNickname(normalized);
    const user = await this.prisma.user.findUnique({
  ...
  private validateNickname(nickname: string) {
    if (
      ...
      throw new BusinessException(
        BusinessCode.INVALID_NICKNAME,
        'Nickname must be 2-12 characters and contain only Korean, English, numbers, or underscore',
      );
    }
  }
  ...
  async getMyBooks(userId: number, page = 1, size = 20) {
    ...
    return sessions.map((session) => ({
      ...
      reviewText: session.aiVerification?.reviewText ?? null,
